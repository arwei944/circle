...
export const getJson = async <T>(path: string): Promise<T> =>
   parse<T>(await fetch(path, json('GET')));

export const sendJson = async <T>(method: string, path: string, body?: unknown): Promise<T> =>
   parse<T>(await fetch(path, json(method, body)));
